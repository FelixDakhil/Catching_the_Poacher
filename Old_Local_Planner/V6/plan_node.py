...
* Linear velocity is SCALED by heading error – the robot slows down during
  sharp turns so it does not drive into obstacles before finishing the turn.
...
Fixes applied
-------------
  FIX-1: Linear velocity now scales down with angular error (was fixed-wing
         constant speed, which caused collisions mid-turn).
  FIX-2: VFH_THRESHOLD set to 0.35 (was 0.20, briefly tried 0.50).
         0.20 was too low (obstacles at 2.5 m slipped through).
         0.50 was too high (sectors at ~1 m borderline, smoothing bled
         into neighbours and eliminated all valleys → robot stopped).
         0.35 marks obstacles as blocked up to ~1.3 m with room to spare.
  FIX-2b: VFH_SMOOTH_WIN reduced from 3 → 2.  Window=3 spreads each
         blocked sector across ±3 neighbours (7 sectors, ~35°); in
         narrow spaces this wiped out all free valleys.  Window=2 still
         smooths noise without over-spreading.
  FIX-5: _find_valleys wrap-around deduplication: duplicate valley entries
         that appeared when a valley started exactly at index 0 are removed.
...
# FIX-2: set to 0.35 (was 0.20 originally, temporarily raised to 0.50).
# With h = (a - b·d)², the threshold maps to a blocked distance of:
#   d_blocked = (a - sqrt(threshold)) / b  →  (1 - sqrt(0.35)) / (1/3.5) ≈ 1.43 m
# 0.20 let obstacles through beyond ~2.1 m; 0.50 blocked at ~0.97 m which,
# after smoothing, falsely eliminated all valleys in open corridors.
# 0.35 blocks up to ~1.4 m with enough margin for the smoother.
VFH_THRESHOLD       = 0.35
# FIX-2b: reduced from 3 → 2.  Window=3 spreads each sector ±3 neighbours
# (7 sectors, ~35°).  In tight corridors that wiped out all free valleys.
VFH_SMOOTH_WIN      = 2      # smoothing half-window (sectors each side)
VFH_D_MAX           = 3.5    # max sensor range (m)
VFH_ROBOT_RADIUS    = 0.18   # TurtleBot3 Burger radius (m)
VFH_SAFETY_DIST     = 0.65   # extra clearance (m)
VFH_A               = 1.0    # obstacle magnitude coefficient
VFH_B               = 1.0 / VFH_D_MAX

FIXED_LINEAR_VEL    = 0.15   # m/s  – maximum forward speed
MAX_ANGULAR_VEL     = 2.84   # rad/s
ARRIVAL_RADIUS      = 0.20   # m – goal considered reached inside this distance

# FIX-1: minimum linear speed fraction when turning hard (0.0 = full stop on
# maximum turn; increase if the robot gets stuck spinning on the spot).
MIN_SPEED_FRACTION  = 0.20   # 20 % of FIXED_LINEAR_VEL at max angular error
...
class VFHPlanner:
    ...
    fixed_speed    : maximum forward velocity (m/s)
    max_angular    : angular velocity limit (rad/s)
    min_speed_frac : fraction of fixed_speed applied when angular error is at
                     its maximum (controls the speed-vs-turn trade-off)
    """
    ...
    def __init__(
        ...
        min_speed_frac: float = MIN_SPEED_FRACTION,
    ) -> None:
    ...
    def _find_valleys(self, binary: list[int]) -> list[tuple[int, int]]:
        ...
        FIX-5: The original double-length trick could produce duplicate
        entries when a valley started at exactly index 0 (the second copy
        of the array would emit the same valley a second time).  We now
        de-duplicate by tracking which start sectors have already been
        recorded.
        """
        n        = len(binary)
        valleys  = []
        ...
        for i in range(2 * n):
            if extended[i] == 0 and start is None:
                ...
                if start < n:
                    norm_start = start % n
                    if norm_start not in seen_starts:
                        seen_starts.add(norm_start)
                        valleys.append((norm_start, (i - 1) % n))
                start = None

        if start is not None and start < n:
            norm_start = start % n
            if norm_start not in seen_starts:
                valleys.append((norm_start, (2 * n - 1) % n))
    ...
    # ------------------------------------------------------------------
    # Velocity output  –  speed-scaled by heading error
    # ------------------------------------------------------------------
    def _to_velocities(self, direction: float | None) -> tuple[float, float]:
        """
        FIX-1: Linear velocity is NO LONGER fixed.  It scales down
        proportionally with the magnitude of the angular error so the
        robot slows while turning and cannot drive into an obstacle before
        the turn is complete.

        At zero error  → full fixed_speed.
        At maximum turn → min_speed_frac * fixed_speed.

        If no valley is found, stop and spin slowly to find an opening.
        """
        if direction is None:
            # Completely blocked – stop and turn slowly to find a valley
        ...
        # FIX-1: scale linear speed down as turn ratio increases
        turn_ratio  = abs(angular_vel) / self.max_angular          # 0.0 → 1.0
        speed_scale = 1.0 - (1.0 - self.min_speed_frac) * turn_ratio
        linear_vel  = self.fixed_speed * speed_scale
...
class PlanNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("min_speed_frac", MIN_SPEED_FRACTION)
        ...
        self._planner = VFHPlanner(
            ...
            min_speed_frac= self.get_parameter("min_speed_frac").value,
        )
        ...
        self.get_logger().info(
            "PlanNode ready  |  VFH+ (speed-scaled)  |  "
