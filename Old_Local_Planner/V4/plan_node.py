...
VFH_SAFETY_DIST     = 0.25   # extra clearance (m)
...
class VFHPlanner:
    ...
    def _to_velocities(self, direction: float | None) -> tuple[float, float]:
        ...
        If no valley is found, keep moving and spin to find an opening.
        """
        if direction is None:
            # Completely blocked – keep speed, turn hard to find a valley
            return self.fixed_speed, self.max_angular * 0.6
...
class PlanNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._scan_sub = self.create_subscription(
            Float32MultiArray, "/processed_scan", self._scan_cb, 10,
        )
