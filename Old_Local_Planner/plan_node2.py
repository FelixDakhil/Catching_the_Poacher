...
class PlanNode(Node):
    ...
    def _build_histogram(self, ranges: np.ndarray) -> np.ndarray:
        ...
        Sectors are then inflated by _inflation_deg to add a safety margin.
        """
        ...
        # Inflate blocked sectors — spread danger to neighbouring sectors
        degs_per_sector = 360 // self.NUM_SECTORS
        inf_sectors = max(1, int(self._inflation_deg / degs_per_sector))
    ...
    def _select_heading(
        ...
        """
        FREE_THRESHOLD = self._free_thresh  # sectors below this = passable
